...
std::vector<cv::KeyPoint>* VisionGearAlpha::getfindBlobsOutput(){
	 ...
	 */
	void VisionGearAlpha::cvResize(cv::Mat &src, cv::Size &dSize, double fx, double fy, int interpolation, cv::Mat &dst) {
	...
	void VisionGearAlpha::hslThreshold(cv::Mat &input, double hue[], double sat[], double lum[], cv::Mat &out) {
	 ...
	 * @param dst Output Image.
	 */
	void VisionGearAlpha::cvErode(cv::Mat &src, cv::Mat &kernel, cv::Point &anchor, double iterations, int borderType, cv::Scalar &borderValue, cv::Mat &dst) {
		 ...
		 * @param output The image in which to store the output.
		 */
		void VisionGearAlpha::mask(cv::Mat &input, cv::Mat &mask, cv::Mat &output) {
	...
	void VisionGearAlpha::findBlobs(cv::Mat &input, double minArea, double circularity[], bool darkBlobs, std::vector<cv::KeyPoint> &blobList) {
		blobList.clear();
